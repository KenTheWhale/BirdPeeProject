 ...
 * @author Admin
 */
public class SOMailSend extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
           ...
           BirdPeeDAO.SHOPOWNER_sendEmail(shopID, customerID, orderID);
           request.setAttribute("sid", shopID);
           request.setAttribute("cid", customerID);
           request.setAttribute("oid", orderID);
           request.getRequestDispatcher("SendOrderMail").forward(request, response);
        } catch(Exception e){
            e.printStackTrace();
        }
    }
